...
public partial class TestChargeCalculator
{
    ...
    {
        Assert.Equal(expectedFee, new ChargeCalculator().CalculateFee(GetCalculation6Parameters(carType)));
    }

    [Fact]
    public void Example7()
    {
        Assert.Equal(11m, new ChargeCalculator().CalculateFee(GetExample7Parameters(new DateTime(2022, 01, 14))));
        Assert.Equal(20m, new ChargeCalculator().CalculateFee(GetExample7Parameters(new DateTime(2021, 01, 13))));
        Assert.Equal(20m, new ChargeCalculator().CalculateFee(GetExample7Parameters(new DateTime(2021, 01, 12))));
    }
    ...
    {
        Assert.Equal(2, new ChargeCalculator().CalculateFee(GetExample9Parameters()));
    }
}